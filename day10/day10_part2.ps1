    ...
    if ([int][string]$grid["$x,$y"] -eq $currentNumber) {
        ...
        $dir.keys | ForEach-Object { 
            ...
            if ([int][string]$grid["$($x + $x1),$($y + $y1)"] -eq ($currentNumber + 1) ) {
                test-paths -x ($x + $x1) -y ($y + $y1) -maxX $maxX -maxY $maxY -currentNumber ($currentNumber + 1) -direction $_
            }
        }
    }
}
...
$sum = for ($i = 0; $i -lt $startpositions.count; $i++) {
    ...
    test-paths -x $x -y $y -maxX $data[0].Length -maxY $data.count -currentNumber 0 -direction 'start'
}
